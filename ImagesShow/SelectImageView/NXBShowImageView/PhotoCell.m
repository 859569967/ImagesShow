...
- (void)layoutSubviews{
    ...
    _scroll.frame = CGRectMake(0, 0, Screen_width, Screen_height);
    _scroll.contentSize = CGSizeMake(Screen_width, h);
    
    _imageView.frame = CGRectMake(0, (Screen_height-h)/2, Screen_width, h);
...
- (void)contentScroller{
    ...
    if(f.size.height < size.height){
        f.origin.y = (Screen_height - f.size.height)/2;
    }else{
        f.origin.y = 0.0f;
    }
    _imageView.frame = f;
